...
<template>
    <section class="flex flex-col h-full gap-4 p-4">
        <header
            class="px-4 flex flex-col items-center justify-center gap-4 w-full">
            <div class="flex flex-col items-center text-center w-full">
                <div class="relative w-48 h-48 rounded-lg overflow-hidden group mb-4"
                ...
                <h1 class="text-2xl md:text-3xl font-bold text-black dark:text-white select-none truncate max-w-full"
                ...
                <p v-if="playlist?.description"
                    class="text-sm text-neutral-600 dark:text-neutral-300 line-clamp-2 max-w-[16rem]">
        ...
        <div class="flex flex-col overflow-y-auto overflow-x-hidden pb-36">
            <template v-if="props.uuid === '00000000-0000-0000-0000-000000000000'">
                <MobilePlaylistSong v-for="(song, index) in player.likedSongList" :key="song.uuid" :index="index + 1"
                    :song="song" :playlistUuid="props.uuid" />
            </template>
            <template v-else>
                <MobilePlaylistSong v-for="(song, index) in playlist?.songs ?? []" :key="song.uuid" :index="index + 1"
                    :song="song" :playlistUuid="props.uuid" />
            </template>
        </div>
    </section>
