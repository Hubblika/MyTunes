...
<template>
    <article class="w-72 flex flex-col gap-0.5">
        <div :class="['w-full aspect-square border rounded-md', { 'bg-[#ff00ff20]': isLikedSongs }]">
        ...
        <span v-if="!!playlist" class="text-gray-600 line-clamp-1 w-full overflow-ellipsis"><span class="font-semibold">{{ playlist.name }}</span> &middot; n songs</span>
        <span v-else class="text-gray-600 line-clamp-1 w-full overflow-ellipsis"><span class="font-semibold">{{ song!.title }}</span> &middot; [Artist name]</span>
    </article>
</template>
